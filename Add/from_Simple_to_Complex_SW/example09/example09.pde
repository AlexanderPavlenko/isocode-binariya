...
{                           // теперь всё само подстраивается под радиусы: внутренний и внешний   
  ...
  { 
    float z = 1.0, k = 0.5;
    if (rOut - rIn < 50) z = 0.75;  
    if (rOut < 150) { z = 0.75; k = 0.25; }
    ...
    pushMatrix();  // вращаем теперь через rotate(...), ну и translate(...) - тоже понадобился
      ...
      pushMatrix();
        ...
        fill(cb); drawCirc(0, 0, r2pz + k);
        fill(c3); drawCirc(0, 0, r2pz);
        fill(cb); drawCirc(0, 0, r2mz);
        fill(c2); drawCirc(0, 0, r2mz - k);
        fill(cb); drawCirc(0, 0, r1);
...
{   
  ...
  background(255);
  swR1.draw(width / 2, height / 2, 400, 420); 
  swR2.draw(width / 2, height / 2, 300, 350); 
  swR3.draw(width / 2, height / 2, 150, 250);
