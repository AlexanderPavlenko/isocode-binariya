...
{   
  ...
  { 
    ...
    pushMatrix();  // вращаем теперь через rotate(...), ну и translate(...) - тоже понадобился
      ...
      fill(mcColor(ci, 1.1, 0)); drawCirc(- 0.04 * r, - 0.04 * r, r * 0.7);
      fill(mcColor(ci, 1.2, 0)); drawCirc(- 0.07 * r, - 0.07 * r, r * 0.6);
      
      fill(mcColor(ci, 1.25, 0)); drawCirc(- 0.25 * r, - 0.25 * r, r * 0.1);
...
void setup()
{ 
  if (fullScreen) size(displayWidth, displayHeight); else size(800, 800); 
  frameRate(30);
