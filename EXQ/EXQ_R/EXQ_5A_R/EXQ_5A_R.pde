...
{ 
  ...
  c3 = blend(c1, c2, 60);
  println("blend with k = 60%", red(c3), green(c3), blue(c3));
...
color blend(color a, color b, int k)
{
  return color(red(a) + (red(b) - red(a)) * k / 100,
               green(a) + (green(b) - green(a)) * k / 100,
               blue(a) + (blue(b) - blue(a)) * k / 100
              );  
}
