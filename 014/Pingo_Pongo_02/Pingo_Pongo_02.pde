...
int resX = 1280, resY = 800;                  // размер изображения
...
PShape sh;                                    // объект для векторых форм  
...
class c_ramp                      
{
  ...
  {                                            // перемещаем
    ...
    fill(cc); rect(x + xH, y, t, w); circle(x + xH, y - w / 2, t / 2); circle(x + xH, y + w / 2, t / 2);
    float tt = t * 0.75; 
    fill(cc2); rect(x + xH, y, tt, w); circle(x + xH, y - w / 2, tt / 2); circle(x + xH, y + w / 2, tt / 2);
    float yy = y + w / 2; tt = t * 0.15; fill(cc); // рисуем количество жизней 
    for (int i = 1; i <= l; i++)
    { if (i == 1) circle(x + xH, yy, tt * 2);
      else circle(x + xH, yy - tt - (i - 1) * (w - tt) / (ml - 1), t * 0.1 * (0.35 + arcin((i - 1.0) / (l - 1)))); 
    }  
...
{ 
  ...
  rectMode(CENTER); shapeMode(CENTER); textAlign(CENTER);  // смотрим Reference
  ...
  sh = loadShape("data\\pingo_pongo.svg");          // загружаем "заставку" 
} 
...
void draw()
{
  ...
  if (start) shape(sh, resX / 2, resY / 2, 2.5 * 195 * scx, 2.5 * 22 * scx);
  else D += 0.02 * sc;
}
