...
class c_drawUse
{
  ...
  float seg[], d1x[], d1y[], d2x[], d2y[];

  c_drawUse() { seg = new float[92]; d1x = new float[368]; d1y = new float[368]; d2x = new float[368]; d2y = new float[368]; scale = 1.0; }
...
void drawRoundRay(float x1, float y1, float x2, float y2, float w1, float w2)
{
  ...
  x = x1 + (drawUse.d1x[0] = dx1); y = y1 + (drawUse.d1y[0] = dy1);
  if (gf) { beginShape();
            vertex(x, y);
          }  
  s1 = ceil(a * drawUse.cSegs(w1) / TWO_PI);
  ad1 = a / s1; a1 = drawUse.xyAngle(dx1, dy1); a2 = a1 - ad1;
  for (i = 1; i <= s1; i++) { x = x1 + (drawUse.d1x[i] = w1 * cos(a2)); y = y1 + (drawUse.d1y[i] = - w1 * sin(a2));
                              if (gf) vertex(x, y); 
                              a2 -= ad1;
                            }
  a3 = b + drawUse.xyAngle(dx2, dy2);
  x = x2 + (drawUse.d2x[0] = dx2 = w2 * cos(a3)); y = y2 + (drawUse.d2y[0] = dy2 = -w2 * sin(a3));
  if (gf) vertex(x, y);
  s2 = ceil(b * drawUse.cSegs(w2) / TWO_PI);
  ad2 = b / s2; a4 = a3 - ad2;
  for (i = 1; i <= s2; i++) { x = x2 + (drawUse.d2x[i] = w2 * cos(a4)); y = y2 + (drawUse.d2y[i] = - w2 * sin(a4));      
                              if (gf) vertex(x, y);
                              a4 -= ad2;
                            }
  ...
  } else for (i = 1; i <= s1; i++) vertex(x1 + drawUse.d1x[i] * dr, y1 + drawUse.d1y[i] * dr);
  dr = 1 + wr / w2; r = w2 + wr;
  ...
  } else for (i = 0; i <= s2; i++) vertex(x2 + drawUse.d2x[i] * dr, y2 + drawUse.d2y[i] * dr);
  if ((w1 > wr) || (w2 >  wr))
  { vertex(x, y);
    d = 1.0 - wr / w1;
    ...
    } else for (i = s2; i >= 0; i--) vertex(x2 + drawUse.d2x[i] * dr, y2 + drawUse.d2y[i] * dr);
    dr = d; r = w1 - wr;
    if (rc1)
    { a2 += ad1;
      for (i = s1; i > 0; i--) { vertex (x1 + r * cos(a2), y1 - r * sin(a2)); a2 += ad1; }
    } else for (i = s1; i > 0; i--) vertex(x1 + drawUse.d1x[i] * dr, y1 + drawUse.d1y[i] * dr);/**/     
    vertex(x, y);       
  }
  endShape(CLOSE);
  popStyle();    
