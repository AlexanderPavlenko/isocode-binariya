/////////////////////////////////////////////// все о шариках
float offsx = 0, offsy = 0, offsc = 0.1, speed = 1.15, radius = 40, reaction = 0.2, falloff = 0.975, border = 1.0;
int count = 850;
...
class c_circ
{
  ...
  void draw(int i, float px, float py, float r)
  {
    float d = dist(xx, yy, px, py);
    if (d > r) return;
    float rr = rad;
    rad *= 1 - d / r;
    draw(i); 
...
{   
  ...
  { circ[i].regenerate();
    circ[i].draw(i, pcirc.xx, pcirc.yy, 200);
  }  
}
