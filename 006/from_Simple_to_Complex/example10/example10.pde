...
class c_emouse
{
  ...
  {
    x = y = dx = dy = m = 0; react = r; foff = f; sw = true;
  }
  void regenerate()
  { 
    float d;
    if (mousePressed)
    { if (sw)
      { d = dist (px, py, mouseX, mouseY); if (d < 0.0001) d = 1;
        x = mouseX; y = mouseY; m = 0.05;
        dx = 0.5 * (mouseX - px) / d;
        dy = 0.5 * (mouseY - py) / d;
        sw = false; px = mouseX; py = mouseY; return;
      }
      d = dist (x, y, mouseX, mouseY); if (d < 0.0001) d = 1;
      dx += (mouseX - x) / d * react;
      dy += (mouseY - y) / d * react;
      dx *= foff; dy *= foff;
      x += dx; y += dy;
    ...
    dx *= 0.99999; dy *= 0.99999; m *= 0.9999;
    x += dx; y += dy;
    px = mouseX; py = mouseY;
  }
}
...
{ 
  ...
  emouse = new c_emouse(0.001, 0.975);
}
